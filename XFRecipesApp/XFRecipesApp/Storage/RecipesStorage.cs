...
namespace XFRecipesApp.Storage
{
    public static class RecipesStorage
    {
        static List<RecipesModel> _allRecipes;

        public static List<RecipesModel> AllRecipes
        {
            get
            {
                if (_allRecipes == null)
                {
                    _allRecipes = new List<RecipesModel>
                    {
                        new RecipesModel()
                        {
        ...
        static List<RecipesGroupingModel> _allRecipesGroup;

        public static List<RecipesGroupingModel> AllRecipesGroup
        {
            get
            {
                if (_allRecipesGroup == null)
                {
                    var easyGrouping = new RecipesGroupingModel("Easy","E");
                    easyGrouping.AddRange(AllRecipes.Where
                        (
                            r => r.Difficulty == Difficulty.Easy
                        )
                    );

                    var mediumGrouping = new RecipesGroupingModel("Medium","M");
                    mediumGrouping.AddRange(AllRecipes.Where
                        (
                            r => r.Difficulty == Difficulty.Medium
                        )
                    );

                    var hardGrouping = new RecipesGroupingModel("Hard","H");
                    hardGrouping.AddRange(AllRecipes.Where
                        (
                            r => r.Difficulty == Difficulty.Hard
                        )
                    );

                    _allRecipesGroup = new List<RecipesGroupingModel>()
                    {
                        easyGrouping,
                        mediumGrouping,
                        hardGrouping
